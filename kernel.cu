...
__device__ double maximumLikelihoodEstimationLog(int length,
	...
	int length, int* Ms, int* Rs, int precision, int* freeMem)
{
	...
	for (int dataIdx = kernelIdx; dataIdx < nAll; dataIdx += TOTAL_THREAD_NUM)
	{
		...
		data[dataIdx] = maximumLikelihoodEstimationLog(length, Ms, Rs, Hs, precision);
	}
}

int sumIntArr(int length, int* Ms, int* Rs) {
	int sum = 0;
	...
	int length, int* Ms, int* Rs)
{
	...
	mlae_kernel __KERNEL_ARGS__ (d_data, nAll, length, d_Ms, d_Rs, sumIntArr(length, Ms, Rs), freeMem);
	checkCudaErrors(cudaMemcpy(data, d_data, sizeof(double) * nAll, cudaMemcpyDeviceToHost));
